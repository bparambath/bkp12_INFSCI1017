package bkp12_Music;

public class MusicTester {

	public static void main(String[] args) {
		
		//testing Song
		Song s = new Song("1");
//		s.setLength(119);
//		s.setFilePath("songs/cometogether.mp3");
//		s.setRecordDate("1993-10-23 00:00:00");
//		s.setReleaseDate("1994-10-23 00:00:00");
//		s.setTitle("Flamingo");
//		System.out.println(s.getFilePath());
//		Song s2 = new Song("Flamingoo",432,"songs/cometogetherrrrr.mp3","1994-10-23 00:00:00","1995-10-23 00:00:00");
//		
//		//testing Artist
//		Artist a = new Artist("1");
//		a.setFirstName("Kero Kero");
//		a.setLastName("Bonito");
//		a.setBandName("Kero Kero Bonito");
//		a.setBio("Funny song lol");
//		System.out.println(a.getBio());
//		Artist a2 = new Artist("Meek","Mill","Meek Mill");
//		
//		//testing Album
//		Album al = new Album("1");
//		al.setTitle("Bonito Generation");
//		al.setReleaseDate("1994-10-23 00:00:00");
//		al.setCoverImagePath("images/revolver.jpg");
//		al.setRecordingCompany("Bonitoo");
//		al.setNumberOfTracks(5);
//		al.setPmrcRating("R");
//		al.setLength(234);
//		Album al2 = new Album("Bonito Generationn", "1995-10-23 00:00:00", "images/revolvers.jpg", "bonito", 3, "X", 752);
//		
//		
	}

}
